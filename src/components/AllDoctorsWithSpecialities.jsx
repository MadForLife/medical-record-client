...
const AllDoctorsWithSpecialities = () => {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      if (isAuthenticated) {
        try {
          const apiUrl = `http://localhost:8080/api.medical-record/v1/doctors/with-specialities`; // Replace with actual API endpoint
  ...
  return (
    <Container className="mt-5">
      <h2>Doctor List with Specialties</h2>
      {doctors.length > 0 ? (
          ...
          <tbody>
            {doctors.map((doctor, index) => (
              <tr key={doctor.id}>
                ...
                <td>
                  {doctor.doctorSpecialities.length > 0 ? (
...
export default AllDoctorsWithSpecialities;
